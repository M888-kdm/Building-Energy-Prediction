...
def test_get_experiment_id():
    ...
    with patch(
        ...
        "mlflow.create_experiment", return_value="new_id"
    ) as mock_create_experiment:
        exp_id = get_experiment_id(exp_name)
        assert (
            exp_id == "new_id"
        ), "Experiment ID creation did not return the expected ID."

    # Case where experiment exists
    mock_experiment = MagicMock()
    mock_experiment.experiment_id = "existing_id"
    with patch(
        "mlflow.get_experiment_by_name", return_value=mock_experiment
    ) as mock_get_experiment_by_name:
        exp_id = get_experiment_id(exp_name)
        assert (
            exp_id == "existing_id"
        ), "Existing experiment ID was not correctly returned."
